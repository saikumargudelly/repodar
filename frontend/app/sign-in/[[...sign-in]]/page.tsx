...
const PROVIDERS: { key: string; label: string; icon: string; strategy: OAuthStrategy }[] = [
  { key: "github", label: "Continue with GitHub", icon: "🐙", strategy: "oauth_github" },
  { key: "google", label: "Continue with Google", icon: "🔍", strategy: "oauth_google" },
  { key: "discord", label: "Continue with Discord", icon: "💬", strategy: "oauth_discord" },
];
...
export default function SignInPage() {
  ...
  useEffect(() => {
    if (!signInLoaded || !validProvider || autoTriggered.current || (authLoaded && isSignedIn)) return;
    autoTriggered.current = true;
    void startOAuth(validProvider.key);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [signInLoaded, authLoaded, isSignedIn]);

  const startOAuth = async (providerKey: string) => {
    const provider = PROVIDERS.find((p) => p.key === providerKey);
    if (!provider || !signIn || !signInLoaded) return;
  ...
  const isReady = signInLoaded && !!signIn;
  ...
  return (
    <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "var(--bg-primary)", padding: "24px" }}>
      <div className="auth-card" style={{ width: "100%", maxWidth: "440px", background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "12px", padding: "32px" }}>
        ...
        <h1 style={{ margin: "0 0 6px", fontSize: "24px", fontWeight: 700, letterSpacing: "-0.02em" }}>Welcome back</h1>
        <p style={{ margin: "0 0 24px", color: "var(--text-secondary)", fontSize: "14px" }}>
          Sign in to access your watchlist, alerts, and personalized radar.
        </p>
          ...
          {PROVIDERS.map((provider) => {
            ...
            return (
              ...
              >
                ...
                {loading ? `Redirecting to ${provider.label.replace("Continue with ", "")}…` : provider.label}
              </button>
            );
          })}
        </div>
        ...
        <div style={{ marginTop: "20px", paddingTop: "16px", borderTop: "1px solid var(--border)" }}>
          <p style={{ margin: "0 0 8px", color: "var(--text-muted)", fontSize: "13px" }}>
            New here?{" "}
            <Link href="/sign-up" style={{ color: "var(--accent-blue)", textDecoration: "none", fontWeight: 600 }}>Create an account</Link>
          </p>
